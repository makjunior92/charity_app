function total() {
		...
		var pi = document.getElementById("value1").innerHTML;

		var si = document.getElementById("value3").innerHTML;
		var  mi = document.getElementById("value2").innerHTML;

		var purchase_input = parseInt(pi.replace(/[^a-z\d\s]+/gi, ""));
		var sell_input  = parseInt(si.replace(/[^a-z\d\s]+/gi, ""));
		var mortgage_input = parseInt(mi.replace(/[^a-z\d\s]+/gi, ""));


		console.log("pi= "+purchase_input);

		console.log("si= "+sell_input);


		console.log("mi= "+mortgage_input);

		var purchase_rebate = 0;
		var sell_rebate =0;
		var mortgage_rebate =0;
    var total_charitable_donation = 0;

		if(purchase_input > 100000){
			purchase_rebate = purchase_input*.005;
			document.getElementById("cash_for_buying").innerHTML = "$"+purchase_rebate.toLocaleString();
		}else{
			document.getElementById("cash_for_buying").innerHTML = "$"+0;
		}

		if(sell_input > 100000) {
			sell_rebate = sell_input*.005;
			document.getElementById("cash_for_sell").innerHTML = "$"+sell_rebate.toLocaleString();
		}else{
			document.getElementById("cash_for_sell").innerHTML = "$"+0;
		}

		console.log("sr= "+sell_rebate);




		if(mortgage_input > 100000) {
			mortgage_rebate = mortgage_input*.005;
			if(mortgage_rebate>2500)
				mortgage_rebate = 2500;
			document.getElementById("closing_cost").innerHTML = "$"+mortgage_rebate.toLocaleString();
		}else{
			document.getElementById("closing_cost").innerHTML = "$"+0;
		}



      var donation_for_purchase;
      if(purchase_input<100000){
        donation_for_purchase = 0;
      }
      else if (purchase_input>=100000 && purchase_input<499000) {
        donation_for_purchase = 250;
      }
      else if (purchase_input>=500000 && purchase_input<750000) {
        donation_for_purchase = 500;
      }
      else if (purchase_input>=750000 && purchase_input<1000000) {
        donation_for_purchase = 750;
      }
      else {
        donation_for_purchase =1000;
      }


     var donation_for_sell;
      if(sell_input<100000){
        donation_for_sell = 0;
      }
      else if (sell_input>=100000 && sell_input<499000) {
        donation_for_sell = 250;
      }
      else if (sell_input>=500000 && sell_input<750000) {
        donation_for_sell = 500;
      }
      else if (sell_input>=750000 && sell_input<1000000) {
        donation_for_sell = 750;
      }
      else {
        donation_for_sell =1000;
      }


     total_charitable_donation = donation_for_purchase+donation_for_sell;


      document.getElementById('charitable-donation').innerHTML = "$"+ total_charitable_donation.toLocaleString();
...
(function (factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        var newLineAndTabRegexp = new RegExp('https://www-nestbenefit-com.usrfiles.com/[\\n\\t]/', 'g');
        ...
        function Plugin(element, options) {
            ...
            this.tool.className = 'tooltip';

            this.handle = document.createElement('div');
            this.handle.className = this.options.handleClass;
			this.tool.innerHtml = this.value;
			console.log(this.value);
			this.handle.appendChild(this.tool);
    ...
    (function () {
        ...
        function valueOutput(element) {
		    ...
		    var value_format = value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            output.innerHTML = '$' + value_format;
			var rangeslider = element.parentNode.getElementsByClassName('tooltip')[0];
        ...
        rangeSlider.create(elements, {
            ...
            // Callback function
            onInit: function () {
				 document.getElementsByTagName("h1")[0].innerHTML = '$5,000';
          		 document.getElementsByTagName("h1")[1].innerHTML = '$2,500';
        		 document.getElementsByTagName("h1")[4].innerHTML = '$500';



				document.querySelector("#js-rangeSlider-0 .rangeSlider__handle .tooltip").innerHTML = "$500,000";
				if(document.querySelector("#js-rangeSlider-1") != null){
					document.querySelector("#js-rangeSlider-1 .rangeSlider__handle .tooltip").innerHTML = "$400,000";
				}
				var w = window.innerWidth;
				if(w > 767){
				document.querySelector("#js-rangeSlider-0 .rangeSlider__handle").style.transform = "translateX(500%)";
				if(document.querySelector("#js-rangeSlider-1") != null){
					document.querySelector("#js-rangeSlider-1 .rangeSlider__handle").style.transform = "translateX(400%)";
				}

				}else{
				document.querySelector("#js-rangeSlider-0 .rangeSlider__handle").style.transform = "translateX(50px)";
				if(document.querySelector("#js-rangeSlider-1") != null){
					document.querySelector("#js-rangeSlider-1 .rangeSlider__handle").style.transform = "translateX(40px)";
				}
				}
				document.getElementById('value1').innerHTML = "$500,000";
				document.getElementById('value2').innerHTML = "$400,000";
            },
            ...
            onSlide: function (value, percent, position) {
				...
				var purchase_price = document.getElementById('value1').innerHTML;
				var loan_amount = document.getElementById('value2').innerHTML;
				var sale_price = document.getElementById('value3').innerHTML;
              

				document.getElementsByClassName("tooltip")[0].innerHTML = purchase_price;
				document.getElementsByClassName("tooltip")[1].innerHTML = loan_amount;
				document.getElementsByClassName("tooltip")[2].innerHTML = sale_price;
            ...
            onSlideEnd: function (value, percent, position) {
                ...
                var purchase_rebate = document.getElementById("cash_for_buying").innerHTML;
                var sale_rebate = document.getElementById("cash_for_sell").innerHTML;
                
                var closing_cost_credit = document.getElementById("closing_cost").innerHTML;
                var charitable_donation = document.getElementById('charitable-donation').innerHTML;
              
                
                
                purchase_rebate = purchase_rebate.replace(/[^a-z\d\s]+/gi, "");
                purchase_rebate = parseInt(purchase_rebate);
                
                sale_rebate = sale_rebate.replace(/[^a-z\d\s]+/gi, "");
                sale_rebate = parseInt(sale_rebate);
                
                
                charitable_donation = charitable_donation.replace(/[^a-z\d\s]+/gi, "");
                charitable_donation = parseInt(charitable_donation);

				
                closing_cost_credit = closing_cost_credit.replace(/[^a-z\d\s]+/gi, "");
                closing_cost_credit = parseInt(closing_cost_credit);
                
               
                
                purchase_price = purchase_price.replace(/[^a-z\d\s]+/gi, ""); //these are not necessary
				loan_amount  = loan_amount.replace(/[^a-z\d\s]+/gi, "");  //these are not necessary
				sale_price = sale_price.replace(/[^a-z\d\s]+/gi, "");  //these are not necessary
                
                
                
                
                
				var savings = purchase_rebate + sale_rebate + closing_cost_credit  + charitable_donation;
                
                
				var savings_format = savings.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
				document.getElementsByTagName("h1")[0].innerHTML = '$' + savings_format;
